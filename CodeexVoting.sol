...
contract CodeexVoting {
    ...
    function registerVoting(uint256 _id,uint256 minDeposit, uint256 endRegisterTime, uint256 endVotingTime ) public {
        ...
        voting[_id] = sVoting(_id,minDeposit ,0 ,0 ,0 ,endRegisterTime ,endVotingTime ,true);
    }
    ...
    function withdraw(uint256 _id) public payable {
        if(voting[_id].totalVotes == 0) {
            payable(address(msg.sender)).transfer(linkVoter[_id][msg.sender].depositAmnt);
        } 
        else {
            uint256 amnt;
